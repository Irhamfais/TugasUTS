...
export function calculateResults() {
  ...
  QUESTION_GROUPS.forEach((group) => {
    ...
    if (values.length > 0) {
      categoryAverages[group.category] =
...
export function displayResults(percentage) {
  ...
  if (percentage >= 80) {
    label = "Eco Warrior! 🌟";
    description =
      "Luar biasa! Anda sudah sangat peduli lingkungan dan menjadi contoh yang baik.";
  } else if (percentage >= 60) {
    label = "Eco Conscious 🌿";
    description =
      "Bagus! Anda sudah cukup peduli lingkungan, tapi masih ada ruang untuk berkembang.";
  } else if (percentage >= 40) {
    label = "Eco Beginner 🌱";
    description =
      "Tidak buruk! Anda sudah mulai peduli, mari tingkatkan lagi usaha Anda.";
  } else {
    label = "Eco Starter 🌾";
    description =
      "Mari mulai perjalanan ramah lingkungan Anda! Ada banyak hal yang bisa diperbaiki.";
  }
