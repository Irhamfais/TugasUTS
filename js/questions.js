		...
		group.items.forEach((item, itemIdx) => {
			...
			const navButtons = `
				<div class="btn-navigation">
					${index > 0 ? '<button class="btn btn-back" onclick="prevQuestion()">Kembali</button>' : ''}
					${index < countAllQuestions() - 1 ? '<button class="btn btn-next" onclick="nextQuestion()" disabled>Selanjutnya</button>' : '<button class="btn btn-next" onclick="calculateResults()" disabled>Lihat Hasil</button>'}
				</div>
			`;
